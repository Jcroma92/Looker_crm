...
explore: mda_ac_datos_fmes_rep {
  ...
  join: mda_ac_dia_rep {
    ...
    type: inner
    sql_on: ${mda_ac_dia_rep.mi_fecha_proc_date} = ${mda_ac_datos_fmes_rep.mi_fecha_fin_mes_date} and ${mda_ac_dia_rep.cod_nrbe_en} = ${mda_ac_datos_fmes_rep.cod_nrbe_en} and ${mda_ac_dia_rep.num_sec_ac} = ${mda_ac_datos_fmes_rep.num_sec_ac};;
  }
  join: td_garantia {
    relationship: one_to_many
    type: inner
  ...
  join: dim_tiempo {
    relationship: one_to_many
    type: inner
    sql: SELECT fecha, year, month, meses_evol, case when meses_evol = fecha then fecha else null end as ultimo_mes FROM output.DIM_TIEMPO dim, UNNEST(dim.ultimos_12_meses) as meses_evol
